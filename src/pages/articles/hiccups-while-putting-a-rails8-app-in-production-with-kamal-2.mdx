...
## TLDR:
* You need a server. A basic Digital Ocean Droplet is $12/mo is good enough to start. You might even be able to [get $200 in credits](https://cloud.digitalocean.com/registrations/new?refcode=f6fcd01aaffb) to try it out.
* Important: DON'T setup the server before hand. Just create it and be able to ssh into it.
* You need a Docker image registry. I used [Docker Hub](https://hub.docker.com).
* Once you have a registry access via a personal access token, update you config/deploy.yml and Kamal will handle the creation of the Docker image.
